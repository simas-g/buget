...
export default function InfoCard({ title, description, icon, list, id }) {
  ...
  return (
    <div className="group relative w-full">
      {/* Outer glow effect */}
      <div className={`absolute -inset-0.5 bg-gradient-to-r ${gradientColor} rounded-xl blur opacity-30 group-hover:opacity-60 transition duration-500`}></div>
      ...
      <div className={`relative h-full bg-[#0A0A20]/90 backdrop-blur-xl rounded-xl border border-primary/20 overflow-hidden transform transition-all duration-500 hover:scale-[1.02] hover:-translate-y-2 ${glowColor} hover:shadow-2xl`}>
        ...
        <div className="absolute bottom-6 left-6 w-12 h-12 bg-gradient-to-br from-accent/15 to-secondary/20 rounded-full blur-lg opacity-40 animate-pulse" style={{animationDelay: '1s'}}></div>
        
        {/* Content */}
        <div className="relative z-10 p-6 space-y-4">
          {/* ID Badge */}
          <div className="flex justify-between items-start">
            <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full bg-black border-gray-400 border text-white text-sm font-bold shadow-lg`}>
              {id + 1}
            </span>
          </div>
          
          {/* Icon */}
          <div className={`inline-flex p-3 rounded-2xl bg-gradient-to-br ${gradientColor} shadow-lg transform hover:scale-110 transition-transform duration-300`}>
            {IconComponent && <IconComponent size={32} className="text-white drop-shadow-lg" />}
          </div>
          ...
          <ul className="space-y-3 pt-2">
            {list.map((item, index) => (
              <li className="flex items-start text-sm text-gray-300 group/item" key={index}>
                <CheckCircle
        ...
        {/* Bottom highlight */}
        <div className={`absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r ${gradientColor} opacity-60`}></div>
      </div>
    </div>
  );
}